...
def create_shatter_at(pos, power=1.0):
    ...
    for i in range(n):
        ...
        if random.random() < 0.35:
            mid = ((p1[0] + p2[0]) * 0.5 + random.uniform(-6,6),
                   (p1[1] + p2[1]) * 0.5 + random.uniform(-6,6))
            poly.append(mid)
        poly.append(p2)
    ...
    for poly in shards_local:
        if random.random() < 0.12 and len(poly) >= 3:
    ...
    for poly in final_shards:
        if len(poly) < 3:
            continue
        base = GLASS_COLOR
        col = (base[0] + random.randint(-6,6),
               base[1] + random.randint(-6,6),
               base[2] + random.randint(-6,6),
               base[3])
        s = Shard(poly, col)
